...
- Stack is frozen unless the user revises it: CPython 3.14.7, Bot API 10.3 + `requests`, token only, isolated yt-dlp worker, sqlite + job JSON, `sendDocument` (`file://` on local Bot API). Local/VPS: `getUpdates`. Vercel 24/7: HTTPS webhook (`api/telegram.py`); disk and cookies are ephemeral (`/tmp`).
